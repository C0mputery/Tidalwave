...
namespace Tidalwave.Logger {
    ...
    public static class TidalwaveLogger {
        ...
        private static readonly Dictionary<LogType, LogMethod> logMethods = new(4);
        ...
        /// <summary>Initializes <see cref="TidalwaveLogger"/> with all log types enabled.</summary>
        ...
        /// <summary>Initializes <see cref="TidalwaveLogger"/> with the supplied log methods.</summary>
        ...
        public static void Initialize(LogMethod? debugMethod, LogMethod? infoMethod, LogMethod? warningMethod, LogMethod? errorMethod, bool includeTimestamps, string timestampFormat = "HH:mm:ss") {
            ...
            if (debugMethod != null) { logMethods.Add(LogType.Debug, debugMethod); }
            if (infoMethod != null) { logMethods.Add(LogType.Info, infoMethod); }
            if (warningMethod != null) { logMethods.Add(LogType.Warning, warningMethod); }
            if (errorMethod != null) { logMethods.Add(LogType.Error, errorMethod); }

            TidalwaveLogger.includeTimestamps = includeTimestamps;
            TidalwaveLogger.timestampFormat = timestampFormat;
        }
        ...
        public static void EnableLoggingFor(LogType logType, LogMethod logMethod) { logMethods[logType] = logMethod; }
        ...
        public static void Log(LogType logType, string message) {
            if (logMethods.TryGetValue(logType, out LogMethod logMethod)) {
                if (includeTimestamps) { logMethod($"[{GetTimestamp(DateTime.Now)}]: {message}"); }
                else { logMethod(message); }
            }
        }
        ...
        public static void Log(LogType logType, string logName, string message) {
            if (logMethods.TryGetValue(logType, out LogMethod logMethod)) {
                if (includeTimestamps) { logMethod($"[{GetTimestamp(DateTime.Now)}] ({logName}): {message}"); }
                else { logMethod($"({logName}): {message}"); }
            }
        }
